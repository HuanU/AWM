...
def set_up_walker_mp(route_id, route, intens, xmin, xmax, ymin, ymax, Nx, Ny, x, y, TrailPotential): 
    ...
    destcopy = copy.deepcopy(dest)
    #vel=np.array([0.,0.])
    pos=np.array(start)
    track=np.zeros((wlkr_range,2))

    trailpotweight = 0.0053 # weighting of trail potential over destination potential, started as 0.003, increasing this increases number of stalls
...
#def calc_path_mp():
    ...
    #set the postiion of the walker on its first then subsequent cycles
        ...
        xi=np.array(np.random.normal(0,5,2))
...
#                break
        ...
        if (i%(hist*samp)==0):
            ...
            if stallcounter == 2: # 3 represents the number of stalls before stopping the walker, this will happen with hard obstacles like buildings and water
                dir = [desdirx[int(pos[1])],desdiry[int(pos[0])]]
                perpdir = [desdiry[int(pos[0])],-desdirx[int(pos[1])]]
                dirnorm = dir*(1/np.linalg.norm(dir))
                perpdirnorm = perpdir*(1/np.linalg.norm(perpdir))
                dest[0] = destcopy[0]+20*perpdirnorm[0]-20*dirnorm[0]
                dest[1] = destcopy[1]+20*perpdirnorm[1]-20*dirnorm[1]

                stallcounter+=1 # take notice of the fact that there was a stall
                break
            if stallcounter == 4: # 3 represents the number of stalls before stopping the walker, this will happen with hard obstacles like buildings and water
                ...
                break
            if (np.dot(pos-meanpos,pos-meanpos)<precision) and stallcounter != 2: #this detects 'stalls'
                ...
                #setup_potentials() # setup potentials with lower weight
                DestinationPotential=np.zeros((Nx,Ny))
                grad=np.zeros((2,Nx,Ny))
                grad=trailpotweight*np.array(np.gradient(TrailPotential))
                DestinationPotential=-np.sqrt((dest[0]-x[:,None])**2+(dest[1]-y[None,:])**2)
